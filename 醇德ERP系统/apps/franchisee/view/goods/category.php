...
function edit_programa(node,span){
	$(span).poshytip({
		...
		content:'<div class="actions"><a class="btn btn-mini content-refresh" title="添加子类别" onclick="pub_alert_html(\'<?=$this->url('goods/categoryadd')?>?id='+node.data.key+'\');"><i class="icon-plus"></i></a><a class="btn btn-mini content-remove" onclick="pub_alert_html(\'<?=$this->url('goods/categoryedit')?>?id='+node.data.key+'\');" title="修改"><i class="icon-edit"></i></a><a class="btn btn-mini content-slideUp" title="删除" onclick="pub_alert_confirm(this,\'\',\'<?=$this->url('goods/categorydelete')?>?id='+node.data.key+'\');"><i class="icon-remove"></i></a></div>',
		offsetX: 5,
		offsetY: -32,
		alignTo: 'target',
		alignX: 'right'
				...
				<div class="span4">
					<div class="filetree" id="menutree">
						<ul class="dynatree-container">
							<li id="root" class="expanded">ROOT<ul><?=get_menu_tree($result,null,'uuid')?></ul>
				        </ul>
					</div>
				</div>
			</div>
