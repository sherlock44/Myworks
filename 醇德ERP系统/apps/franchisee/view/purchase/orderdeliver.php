                            ...
                            <?
                                foreach($re as $key=>$val){
                                    ...
                                    <tr>
            							...
            							<td><?=$val['remark']?></td>
            							<td>
     								       <a data-original-title="查看订购商品" rel="tooltip" class="btn btn-small btn-success" href="<?=$this->url('purchase/deliverinfo',array('ordernum'=>$val['ordernum']))?>"><i class="icon-signout"></i></a>
           								  <a href="javascript:pub_alert_confirm(this,'确定已收货？','<?=$this->url("purchase/updateorderstatus")?>?id=<?=$val["id"]?>&status=5');" class="btn btn-small btn-warning" title="完成"><i class="icon-inbox"></i></a>
						                </td>
            						</tr>
                                    <?
                                }
