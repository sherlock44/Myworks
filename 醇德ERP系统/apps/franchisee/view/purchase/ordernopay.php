                            ...
                            <?
                                foreach($re as $key=>$val){
                                    ...
                                    <tr>
            							...
            							<td>
     								       <a data-original-title="查看订购商品" rel="tooltip" class="btn btn-small btn-success" href="<?=$this->url('purchase/nopayinfo',array('ordernum'=>$val['ordernum']))?>"><i class="icon-signout"></i></a>
           								   <a href="javascript:pub_alert_confirm(this,'确定要取消吗？','<?=$this->url("purchase/updateorderstatus")?>?id=<?=$val["id"]?>&status=-1');" class="btn btn-small btn-danger" title="取消"><i class="icon-remove"></i></a>
						                </td>
            						</tr>
                                    <?
                                }
