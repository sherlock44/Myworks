...
<section class="content">
	<div class="row">
		<div class="col-md-12">
									...
									<table class="table table-striped">
										...
										<tbody>
											...
											<tr style="<?if($value['tag']==0){?>background:#EC2085;<?}?>">
												...
												<td>
													<a href="javascript:pub_alert_confirm(this,'确定要删除吗？','<?=$this->url("purchasesr/delgoods")?>?id=<?=$value["id"]?>');" class="btn btn-xs btn-danger" title="删除"><i class="fa fa-trash"></i></a>
												</td>
												<?}else{?>
			...
			<?if($order['status']==0){?>
			<?$url=$this->url("purchasesr/updateorderstatus");?>
			<?if($order['status']==1){?>
			<?$url=$this->url("purchasesr/reupdateorderstatus");?>
			<?}?>
			...
			<?}?>
			<?if($order['status']==1){?>
			<form class="box form-validate form-confirm" action='<?=$this->url("purchasesr/reupdateorderstatus")?>' id="myform" method='post'>
				<div class="box-header with-border">
...
<script>
	function returnList(){
		window.location.href='<?=$this->url("purchasesr/orderconfirm")?>';
	}
