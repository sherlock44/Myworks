...
<section class="content">
	<form  class='box box-primary form-validate' action='<?=$this->url("preferences/updatehouse")?>' id="myform" method='post'>
			...
			<div class="row">
				...
				<div class="col-md-6">
					...
					<div class="form-group">
						...
						<input type="text" name="data[title]" id="title" class="form-control" value="<?=$re['title']?>" data-rule-required="true" data-rule-mobile="false" data-rule-minlength="1">
					</div>
					<div class="form-group">
						...
						<div class="controls">
							<select id="proviceid" name='data[proviceid]' class="form-control" onchange="changesheng(this.value)">
								...
								<option value="<?=$val['id']?>" <?if($val['id']==$re['proviceid']){echo "selected";}?>><?=$val['name']?></option>
								<?}?>
							</select>
							<br />
							<select name='data[cityid]' id="cityid" class="form-control">
								<option value="<?=$val['id']?>" <?if($val['id']==$re['cityid']){echo "selected";}?>><?=$val['name']?></option>
							</select>
						</div>
					</div>
					<div class="form-group">
						<label for="address" class="control-label">地址</label>
						<textarea name="data[address]" class="form-control"><?=$re['address']?></textarea>
					</div>
					<div class="form-group">
						<label for="number" class="control-label">库房楼层</label>
						<div class="input-group">
							<input type="text" name="data[number]" id="number" value="<?=$re['number']?>" class="form-control" data-rule-required="true" data-rule-number="true" data-rule-minlength="1">
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" name="data[rows]" id="rows" value="<?=$re['rows']?>" class="form-control" data-rule-required="true" data-rule-number="true" data-rule-minlength="1">
							<span class="input-group-btn">
								<button class="btn btn-default btn-flat" type="button">只能输入数字</button>
							</span>
						</div>
					...
					<div class="form-group">
						...
						<div class="input-group">
							<input type="text" name="data[cols]" id="cols" value="<?=$re['cols']?>" class="form-control" data-rule-required="true" data-rule-number="true" data-rule-minlength="1">
							<span class="input-group-btn">
								<button class="btn btn-default btn-flat" type="button">只能输入数字</button>
							</span>
						</div>
					...
					<div class="form-group">
						...
						<textarea name="data[remark]" class="form-control"><?=$re['remark']?></textarea>
					</div>
				</div>
			</div>
		</div>
