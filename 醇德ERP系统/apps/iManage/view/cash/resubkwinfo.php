...
<form  class='form-horizontal form-bordered form-validate'	action='<?=$this->url('cash/reinsert')?>'  id="login" name="login" method='post'>
        <div class="row-fluid">
        	<div class="span12">
        				...
        				<table class="table table-hover table-nomargin  table-bordered" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info" style="width: 100%;">
							...
							<td>
								<select name="houseposid[]" id="housepos_<?=$value['id']?>">
									...
									<?foreach($value['pos'] as $va){?>
									<option value="<?=$va['id']?>" <?if($value['houseposid']==$va['id']){echo "selected";}?>><?=$va['title']?></option>
									<?}?>
								</select>
							</td>					
							</tr>
		...
		<div class="box box-color box-bordered">
			...
			<div class="box-content nopadding">
				...
				<div class="control-group">
						...
						<div class="controls">
								...
								<option value="0">选择入库类型</option>
									...
									<option value="<?=$val['id']?>" <?if($rs['rktp']==$val['id']){echo "selected";}?> ><?=$val['title']?></option>
									<?}?>
								</select>
						</div>
				</div>
				...
				<div class="control-group">
						...
						<div class="controls">
							<textarea  rows="3" name="data[inremark]" class="span8" id="outremark"><?=$rs['inremark']?></textarea>
						</div>
				</div>
						...
						<input type="hidden" value="<?=$id?>" name="applyid" >
						 <button type="submit" class="btn btn-primary" >重新提交&nbsp;&nbsp;<i class="icon-shopping-cart"></i></button>    				
