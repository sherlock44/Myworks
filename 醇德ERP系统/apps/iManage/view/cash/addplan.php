...
<script>
	...
	function changesupply(id,goodsuuid,cartid){
		if(id>0){
			$.ajax({
			url:"<?=$this->url('cash/getGoodsSupply')?>",
			data:"id="+id+"&goodsuuid="+goodsuuid+"&cartid="+cartid,
			type:"post",
			success:function(r){

				$("#supplygoods_"+cartid).html(r);
			}

		});
		}else{
			$("#supplygoods_"+cartid).html('');
	...
	<div class="span12">
		<div class="box box-color box-bordered">
			...
			<div class="box-content nopadding">
					...
					<input type='hidden'  name="data[applyid]" value='<?=$applyid?>'>
...
foreach ($goods as $key => $value) {
	?>
                            <tr>
							<td><?=$value['title']?>&nbsp;<?if (!empty($value['imgpath'])) {?><img width=50 height=50   src="<?=$value["imgpath"]?>"><?}
	?></td>
							<td><?=$value['percent']?></td>
							<td><?=$value['specs']?></td>
