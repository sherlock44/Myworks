...
<div class="row-fluid">
        	<div class="span12">
        		<div class="box box-color box-bordered">
					...
					<form  id="formtj" class='form-horizontal form-bordered form-validate'	action='<?=$this->url("newcash/storeupdate")?>'   method='post'>
        				<table class="table table-hover table-nomargin  table-bordered" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info" style="width: 100%;">
							...
							<td><?=$val['depnum']?></td>	
							<td><input type="file" name="files<?=$val['id']?>" id="files<?=$val['id']?>" <?=$val['isproblem']!=0?"disabled":""?> ></td>	
							<td>
								<select name="isproblem[]" class="span10" onchange="changefile(this.value,<?=$val['id']?>)">
									<option value="1" <?=$val['isproblem']==1?"selected":""?>>通过</option>	
									<option value="-1" >有问题，退回</option>	
									<option value="0" >有问题，调整合同</option>	
								</select>
								<input type="hidden" name="id[]"  value="<?=$val['id']?>">
								<input type="hidden" name="contracttitle[]"  value="<?=$val['contracttitle']?>">
								<input type="hidden" name="contractpath[]"  value="<?=$val['contractpath']?>">
							</td>	
						...
						<input type="button" onclick="formti(<?=count($contract)?>)"    class="btn btn-primary" value="提交该<?=count($contract)?>份合同的库房验收结果" >
...
<script>
function formti(num){
	if(!confirm("确认提交该"+num+"份合同的库房验收结果?")){
		return false;
	}
	$("#formtj").submit();

}
function changefile(val,id){
	if(val==0){
		$("#files"+id).removeAttr('disabled');
	}else{
		$("#files"+id).attr('disabled','disabled');
	}
