...
<div class="row-fluid">
        	<div class="span12">
        		<div class="box box-color box-bordered">
					...
					<form  id="formtj" class='form-horizontal form-bordered form-validate'	action='<?=$this->url("newcash/contracttj")?>'   method='post'>
						...
						<input type="hidden" value="<?=$val['id']?>" name="id" id="formid">
						<input type="hidden" value="<?=$applyid?>" name="applyid" >
						</form>		
						<div class="dataTables_paginate paging_full_numbers" id="DataTables_Table_0_paginate">
						<input type="button" onclick="javascript:pub_alert_confirm(this,'该计划共需<?=count($contract)?>份合同，确认都已上传','<?=$this->url("newcash/contractover",array("applyid"=>$applyid))?>');"  class="btn btn-primary" value="提交合同" >
						</div>
        			</div>
        			</div>
        		</div>
