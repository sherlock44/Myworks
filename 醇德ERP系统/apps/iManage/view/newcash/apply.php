        	...
        	<div class="span12">
        		<div class="box box-color box-bordered">
        			<div class="box-title">
                        ...
                        <div class="actions">
        					<a rel="tooltip" data-original-title="添加" href="<?=$this->url('newcash/addapply')?>" class="btn btn-danger"><i class="icon-plus"></i> 添加</a>
        				</div>
        			</div>
                            ...
                            <?
                                foreach($re as $key=>$val){
                                    ...
                                    <tr>
            							...
            							<td><?=$val['cgname']?></td>
            							<td><?=$val['zgname']?></td>
            							<td><span style="<?if($val['status']>0 && $val['status']<12){?>color:red;<?}?>"><?=$sysconf['purchasestatus'][$val['status']]?></td>
										<td><?=date("Y-m-d",$val['created'])?></td>
            							<td>
     								       <a data-original-title="查看" rel="tooltip" class="btn btn-small btn-success" href="<?=$this->url('newcash/editapply',array('id'=>$val['id']))?>"><i class="icon-edit"></i></a>
										   
           								   <a href="javascript:pub_alert_confirm(this,'确定要删除吗？','<?=$this->url("newcash/delapply")?>?id=<?=$val["id"]?>');" class="btn btn-small btn-danger" title="删除"><i class="icon-remove"></i></a>
						                </td>
            						</tr>
                                    <?
                                }
