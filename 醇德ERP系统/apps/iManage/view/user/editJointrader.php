...
<section class="content">
  <form  class='box box-primary form-validate' action='<?=$this->url("user/updatefranchisee")?>'  id="login" name="login" method='post'>
    <div class="box-header with-border">
      <h3 class="box-title">请填写加盟商信息</h3>
    ...
    <div class="box-body">
      <div class="row">
        ...
        <div class="col-md-6">
          ...
          <div class="form-group">
            <label >销售经理</label>
            <select name='data[userid]' class="form-control">
              <?foreach ($admins as $key => $value) {?>
              <option value='<?=$value['id']?>' <?if($re['userid']==$value['id']){?> selected=""  <?}?>   ><?=$value['truename']?></option>
              <?}?>
            </select>
          </div>
          <div class="form-group">
...
//返回列表
function returnList(){
  window.location.href="<?=$this->url('user/joinTrader')?>";
}
</script>
