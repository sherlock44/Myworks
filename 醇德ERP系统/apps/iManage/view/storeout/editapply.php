			...
			<div class="box-content nopadding">
				...
				<?if($re['status']==0 && $re['memberid']==$this->info['id']){?>
					...
					<div class="control-group">
						...
						<div class="controls">
							<input type="text" name="data[fzname]" id="fzname" value="<?=$re['fzname']?>"  class="input-xlarge" data-rule-required="true" data-rule-mobile="false" data-rule-minlength="1">
						</div>
					</div>
					<div class="control-group">
						<label for="mobile" class="control-label">负责人电话</label>
						<div class="controls">
							<input type="text" name="data[fzmobile]" id="fzmobile" value="<?=$re['fzmobile']?>"  class="input-xlarge" data-rule-required="true" data-rule-mobile="false" data-rule-minlength="1">
						</div>
					</div>
				...
				<!--其他流程-->
					...
					<?if($re['status']>=3){?>
						<div class="control-group">
						<label for="number" class="control-label">出库类型<br>
				...
				<div class="control-group">
						<label for="number" class="control-label">出库说明<br>
						
						</label>
						<div class="controls">
								<?=!empty($re) ? $re["outremark"] : ''?>
						</div>
				</div>
					...
					<?}?>
					<div class="control-group">
						<label for="number" class="control-label">申请时间</label>
						<div class="controls">
							<?=date("Y-m-d",$re['created'])?>
						</div>
					...
					<div class="control-group">
						...
						<div class="form-actions">
						<?if($re['status']==1){?>
						<input type="button" onclick="javascript:pub_alert_confirm(this,'开始制定退货单','<?=$this->url("storeout/purchasecheck",array("id"=>$re["id"]))?>');"  class="btn btn-primary" value="<?=$sysconf['storeout'][$re['status']]?>" >
						<?}else if($re['status']==2){?>
						<input type="button" onclick="javascript:window.location.href='<?=$this->url("storeout/selgoods",array("id"=>$re["id"]))?>'" class="btn btn-primary" value="<?=$sysconf['storeout'][$re['status']]?>" class="btn ">
						<?}else if($re['status']==3){?>
						<input type="button" onclick="javascript:pub_alert_confirm(this,'确出库已完成','<?=$this->url("storeout/goodstostoretj",array("id"=>$re["id"]))?>');"  class="btn btn-primary" value="<?=$sysconf['storeout'][$re['status']]?>" >
						<?}else if($re['status']==4){?>
						
						<?}else{?>
						<input type="button" onclick="$('#pub_edit_bootbox').removeClass('fade');$('#pub_edit_bootbox').removeClass('hide');$('#pub_edit_bootbox').attr('tabindex','1');"  class="btn btn-primary" value="<?=$sysconf['storeout'][$re['status']]?>" >
